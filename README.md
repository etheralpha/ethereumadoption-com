...
To get new additions you can use `https://ethereumadoption.com/adoption-new-entries.json`. This is updated daily around midnight UTC+0. Each entry has an `is_recent` attribute. If `is_recent: true` then it's a recent event. If `is_recent: false` then it's a stale event that was previously missed. As time goes on there should be less stale/backdated events being added.
